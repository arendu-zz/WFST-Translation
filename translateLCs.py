...
if __name__ == '__main__':
    ...
    for idx, s in enumerate(open(input_file, 'r').readlines()):
        ...
        os.system('fstcompose %s' % dir + '/lc.s.s.out.sorted.fst data/unk.fst > %s' % dir + '/lc.unk.fst')
        os.system('fstshortestpath -nshortest=' + str(prn) + ' %s' % dir + '/lc.unk.fst > %s' % dir + '/lc.s.s.short.out.fst')
        os.system('fstcompose %s' % dir + '/lc.s.s.short.out.fst data/lm.fst > %s' % dir + '/lc.final.fst')
