...
if __name__ == '__main__':
    ...
    for uni_line in unigrams:
        if uni_line.strip() != '' and len(uni_line.split('\t')) > 1:
            ...
            if bk is not None:
                add_arc_pr(sym_e, lm_fst, to_id, lm_id[NULL], fst.EPSILON, fst.EPSILON, bk)
    ...
    for bi_line in bigrams:
        if bi_line.strip() != '' and len(bi_line.split('\t')) > 1:
            ...
            if bk is not None:
                add_arc_pr(sym_e, lm_fst, to_id, lm_id[ng2], fst.EPSILON, fst.EPSILON, bk)
    ...
    add_arc_pr(sym_e, lm_fst, lm_id[INITIAL], lm_id[NULL], fst.EPSILON, fst.EPSILON, 0.0)
